...
echo blog：查看我的个人博客
set /p about_var=键入你想继续的操作，回车返回主菜单：
rem if %about_var% == update (goto update)
if %about_var% == website (explorer https://club.cnklp.cn)
rem if %about_var% == blog (start https://blog.cnklp.cn)
goto start
...
set  amv_trans_arg=%amv_trans_arg% -i %amv_input%
...
cls
title 正在转换...
ffmpeg %amv_trans_arg% %output%\amvoutput.avi
